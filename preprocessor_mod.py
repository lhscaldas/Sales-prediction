...
class Preprocessor():
    def __init__(self):
        self.oil = pd.read_csv('oil.csv',parse_dates=['date'])
        self.stores = pd.read_csv('stores.csv')
        self.transactions = pd.read_csv('transactions.csv',parse_dates=['date'])
    ...
    def import_train(self):
        ...
        self.series = self.train.pivot(index='date', columns='item', values='sales')
    ...
    def fill_christmas(self):
        ...
        self.series = pd.concat([self.series, missing_df])
        self.series.fillna(0, inplace=True)
        self.series.sort_index(inplace=True)
        self.series = self.series.rename_axis("item", axis="columns")

    def set_available(self):
        zero_sales = list(self.series.loc[:, (self.series == 0).all()].columns)
    ...
    def preprocess_oil(self):
        ...
        oil['date'] = oil['date'].dt.to_timestamp()
        date_range = pd.date_range(start=oil['date'].min(), end=oil['date'].max())
        oil = oil.set_index('date').reindex(date_range).ffill().bfill().reset_index()
        oil.columns = ['date', 'dcoilwtico']
        self.train = self.train.merge(oil, on='date', how='left', suffixes=('', '_oil'))
        # print(oil[oil['dcoilwtico'].isnull()])
        # date_to_dcoilwtico = oil.set_index('date')['dcoilwtico'].to_dict()
        # self.train['dcoilwtico'] = self.train.index.map(date_to_dcoilwtico)
        # self.train = self.train.merge(oil, on='date', how='left')
        # self.test= self.test.merge(oil, on='date', how='left')
        # self.train=self.train.set_index(['date']).sort_index()
        # self.test=self.test.set_index(['date']).sort_index()
...
if __name__ == '__main__':
    ...
    print(preprocessor.train.head())
    print(preprocessor.train.tail())
